...
No phase receives `workbench.yaml` or `task.yaml` as a mounted file. `task.yaml` is parsed by Workbench as control-plane task text; command runners that need detailed task instructions should receive them as ordinary public files such as `/workspace/input/task/input/request.md`. No phase receives a broad archive, spec YAML, job JSON, prompt file, optional support directory, or reward file. Adapter commands receive only the standard request JSON through `WORKBENCH_ADAPTER_REQUEST`. Inputs are read-only and mutations under `/workspace/input` are ignored.
...
Agent prompt text is intentionally thin. Proposers receive only the objective goal, metric, candidate root, traces root, editable paths, and candidate patch output contract. First-party agent runners use adapter-owned `run.with.instructions`, candidate/task/output roots, and the rule that non-internal `/workspace/output` files are persisted for grading and inspection. The first-party rubric grader uses adapter-owned `grade.with.instructions`, the active criterion list, task and runner-output roots, and the minimal score-plus-criteria JSON contract; Workbench wraps that result into `scorecard.json`. File contents are not interpolated into phase prompts; agents and commands inspect the mounted folders directly.
...
- `candidate.path`, `tasks.path`, and `runtime.dockerfile` are workspace-relative literal paths.
...
- `propose`, `run`, and `grade` identify adapters by `use`; all adapter-specific settings, including optional first-party `instructions`, live under `with`.
...
This file is how external adapters report summary text, structured feedback, token usage, and provider cost without adding fields to `workbench.yaml`. Workbench assigns usage to proposer, runner, or grader from the execution purpose.
...
Use shell glue only inside the single command when it genuinely clarifies the phase. Put task dependencies in the runtime Dockerfile instead of installing them during every evaluation job; adapter runtime dependencies belong in adapter `setup` commands.
...
The request carries adapter-specific `with` data and optional resolved auth, but the phase output rules do not change. A single adapter implementation may support propose, run, grade, or any subset by branching on `execution.purpose`.
